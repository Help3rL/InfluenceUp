...
<?php if ( have_comments() ): ?>
	...
	$comments_text   = $comments_number == 1 ? esc_html__( 'Comment', 'cldirectory' ) : esc_html__( 'Comments', 'cldirectory' );
	...
	$comment_args    = [
		'callback'    => 'radiustheme\CLDirectory\Helper::comments_callback',
		'reply_text'  => esc_html__( 'Reply', 'cldirectory' ),
		'avatar_size' => 70,
	];
	?>
    ...
    <div class="block-content mt-30 blog-comment">
        ...
        <div class="comment-box">
			...
			<?php if ( ! comments_open() ) : ?>
                <div class="comments-closed"><?php esc_html_e( 'Comments are closed.', 'cldirectory' ); ?></div>
			<?php endif; ?>
        </div>
    </div>
<?php endif; ?>
...
$comment_form_fields = [
	'author' =>
		...
		. '" placeholder="' . esc_attr__( 'Name', 'cldirectory' ) . ( $rdtheme_req ? ' *' : '' ) . '" class="form-control"' . $rdtheme_aria_req . '></div>',

	'email' =>
		'<div class="col-lg-6 form-group"><input id="email" name="email" type="email" value="' . esc_attr( $rdtheme_commenter['comment_author_email'] )
		. '" class="form-control" placeholder="' . esc_attr__( 'Email', 'cldirectory' ) . ( $rdtheme_req ? ' *' : '' ) . '"' . $rdtheme_aria_req . '></div></div>',
];

$comment_form_args = [
	'class_submit'  => 'submit-btn',
	'submit_field'  => '<div class="form-group submit-button">%1$s %2$s</div>',
	'comment_field' => '<div class="form-group"><textarea id="comment" name="comment" required placeholder="' . esc_attr__( 'Comment *', 'cldirectory' )
