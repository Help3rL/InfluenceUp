...
foreach ($terms as $trm) {
    ...
    $cat_bg=get_term_meta($trm->term_id,'rt_category_color',true) ?:'E7F2FC';
    ...
    $view_post = sprintf(
        /* translators: %s: Category term */
            __('View all posts in %s', 'cldirectory'),
            $trm->name
        );
    if ($settings['rtcl_show_image']) {

        printf('<div class="cat-figure" style="background:#%s">',esc_attr($cat_bg));
    ...
    if (!empty($settings['rtcl_show_count'])) {
        printf("<div class='views'><span class='ads-count'>%d %s</span></div>", absint($count), esc_html__('Ads', 'cldirectory'));
    }
