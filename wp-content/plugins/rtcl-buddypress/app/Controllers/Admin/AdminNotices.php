 ...
 */
namespace RadiusTheme\BP\Controllers\Admin;
...
class AdminNotices {
	...
	public function missing_plugins_warning() {
		...
		foreach ( $this->missing as $title => $url ) {
			...
			if ( count( $this->missing ) === $counter ) {
				...
				$sep = ' ' . esc_html__( 'and', 'rtcl-buddypress' ) . ' ';
			} else {
				$sep = ', ';
			}
			if ( 'Classified Listing' === $title || 'BuddyPress' === $title ) {
		...
		<div class="notice notice-error">
			<p>
				<strong>Classified Listing – BuddyPress Integration Addon</strong> is enabled but not effective. It requires 
				<?php
				echo wp_kses(
					$missing,
					[
	...
	public function old_rtcl_warning() {
		...
		<div class="notice notice-error">
			<p>
				<strong>Classified Listing – BuddyPress Integration Addon</strong> is enabled but some feature will not effective. It is not fully compatible with <a class="thickbox open-plugin-details-modal" href="<?php echo esc_url( $link ); ?>">Classified Listing</a> versions prior <?php echo esc_html( self::MIN_RTCL ); ?>.
			</p>
		</div>
		<?php
	}
