...
class Functions {
	...
	public static function is_tag_disabled() {
		if ( Functions::get_option_item( 'rtcl_moderation_settings', 'hide_form_fields', 'tags', 'multi_checkbox' ) ) {
			return true;
		}
	...
	public static function get_sub_terms_filter_html( $args, $terms = [], $location_query_var = '', $category_query_var = '', $tag_query_var = '' ) {
		...
		if ( ! empty( $terms ) ) {
			...
			if ( empty( $args['parent'] ) && ! empty( $args['instance']['taxonomy_reset_link'] ) ) {
				...
				} elseif ( rtcl()->tag === $args['taxonomy'] && ( ! empty( $args['instance']['current_taxonomy'][ rtcl()->category ] ) || ! empty( $args['instance']['current_taxonomy'][ rtcl()->location ] ) ) ) {
			...
			foreach ( $terms as $term ) {
				...
				if ( $args['taxonomy'] == rtcl()->category ) {
					if ( ! empty( $tag_query_var ) && ! empty( $location_query_var ) ) {
						$term_link = Link::get_listings_page_link() . "$category_base/" . $term->slug . "/$location_base/" . $location_query_var . '/tag/' . $tag_query_var;
					} else if ( ! empty( $location_query_var ) ) {
					...
					if ( ! empty( $tag_query_var ) && ! empty( $category_query_var ) ) {
						$term_link = Link::get_listings_page_link() . "$category_base/" . $category_query_var . "/$location_base/" . $term->slug . '/tag/' . $tag_query_var;
					} else if ( ! empty( $category_query_var ) ) {
					...
					if ( ! empty( $location_query_var ) && ! empty( $category_query_var ) ) {
						$term_link = Link::get_listings_page_link() . "$category_base/" . $category_query_var . "/$location_base/" . $location_query_var . '/tag/' . $term->slug;
					} else if ( ! empty( $category_query_var ) ) {
